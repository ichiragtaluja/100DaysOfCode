...
const checkFunction = ({ a, b, arr }) => {
  let newArr = [];

  a % 15 === 0 ? newArr.push(a) : null;
  b % 15 === 0 ? newArr.push(b) : null;

  const pushIfDivisibleBy15 = (num) =>
    num % 15 === 0 ? newArr.push(num) : null;
...
const extractCompny = ({ company }) => company;
const anonymousFunction = ({ details }) => details.map(extractCompny);
...
const extractCompny = ({ name, company }) => name[0] === "A";

const findCompany = (obj) => obj.company;

const anonymousFunction = ({ details }) =>
