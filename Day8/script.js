...
//Method 2
const incrementNumbers = arrayOfNumbers => arrayOfNumbers.map(individualNumber => individualNumber + 3)
...
const getProductNamesInLowerCase = productsArray => productsArray.map(uncapitaliseNameOfProductInObject)
...
const filterLongStrings = arrayOfStrings => arrayOfStrings.filter(lengthGreaterThan5)
...
const isDivisibleBy3and5 = arrayOfNumbers => arrayOfNumbers.filter(isDivisibleBy15)
...
const filterByAge = arrayOfPersons => arrayOfPersons.filter(isAgeGreaterThan30)
...
const filterByScore = arrayOfStudents => arrayOfStudents.filter(isScoreGreaterThan80)
...
const isCityIndoreAndAgeGreaterThan70 = personDetails => personDetails.age > 70 && personDetails.city === "Indore"

const filterByCityAndAge = arrayOfPeople => arrayOfPeople.filter(isCityIndoreAndAgeGreaterThan70)
...
const filterByGenre = arrayOfBooks => arrayOfBooks.filter(isGenreScienceFiction)
...
// Your ES6 code here
const filterBySalary = (arrayOfEmployees, salary, age) => arrayOfEmployees.filter(person => person.age > age && person.salary > salary)
